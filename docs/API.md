...
`GET /events` supports `q`, `city`, `page`, and `limit`. Event detail is addressed by the event ID used by the frontend route `/events/:id`.
...
| `GET` | `/payments/paystack/callback` | Authenticated | Verify a customer-return payment reference |
...
- `GET /organizer/check-in` — ticket check-in data.
- `POST /organizer/check-in` — validate/check in an active ticket.
...
- `GET /admin/check-in` — check-in management.
- `POST /admin/check-in` — validate/check in a ticket.
