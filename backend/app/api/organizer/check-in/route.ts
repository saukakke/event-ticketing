...
export async function POST(request: Request) {
  try {
    ...
    const ticket = await prisma.ticket.findFirst({ where: { OR: [{ code: input.code }, { qrToken: input.code }] }, include: { event: true, ticketType: true, order: { include: { user: { select: { id: true, name: true, email: true } } } } } });
    ...
    if (ticket.status !== "ACTIVE") return errorResponse("TICKET_INVALID", "This ticket is no longer valid.", 409);
    ...
    const updated = await prisma.ticket.updateMany({ where: { id: ticket.id, checkedIn: false, status: "ACTIVE" }, data: { checkedIn: true, checkedInAt: new Date() } });
    ...
    return ok({ id: result.id, code: result.code, status: result.status, checkedIn: result.checkedIn, checkedInAt: result.checkedInAt, event: result.event, ticketType: result.ticketType, holder: result.order.user });
...
export async function GET() {
  try {
    ...
    const [checkedIn, totalPaid] = await Promise.all([prisma.ticket.count({ where: { eventId: { in: eventIds }, checkedIn: true, status: "ACTIVE", order: { status: "PAID" } } }), prisma.ticket.count({ where: { eventId: { in: eventIds }, status: "ACTIVE", order: { status: "PAID" } } })]);
