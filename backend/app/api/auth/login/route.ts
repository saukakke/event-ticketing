...
export async function POST(request: NextRequest) {
  try {
    ...
    if (user.deletedAt) return errorResponse("ACCOUNT_DELETED", "This account is no longer active.", 403);
    if (user.suspendedAt) return errorResponse("ACCOUNT_SUSPENDED", user.suspensionReason || "This account is currently suspended.", 403);
