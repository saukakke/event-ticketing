...
export async function GET(request: Request) {
  try {
    ...
    if (statusParam && !ORDER_STATUSES.includes(statusParam as OrderStatusValue)) {
      return errorResponse("INVALID_STATUS", "Order status must be PENDING, PAID, FAILED, or REFUNDED.", 400);
    }
    ...
    const [data, total] = await Promise.all([
      prisma.order.findMany({
        ...
        include: {
          ...
          tickets: { select: { id: true, code: true, status: true, checkedIn: true } },
        },
    ...
    return ok({ orders: data, pagination: { page, pageSize, total, pages: Math.ceil(total / pageSize) } });
  } catch (error) {
