    ...
    data: {
      ...
      items: {
        create: {
          ...
          quantity,
          unitPriceKobo: ticketType.priceKobo,
        },
      },
    },
  });
...
async function main() {
  ...
  const organizerTwo = await upsertUser({
    ...
    name: "Northern Events Organizer",
    role: UserRole.ORGANIZER,
    passwordHash,
  });
  ...
  const attendeeTwo = await upsertUser({
    email: "attendee2@eventflow.com",
    name: "Second Demo Attendee",
    role: UserRole.ATTENDEE,
    passwordHash,
  });

  const attendeeThree = await upsertUser({
    email: "attendee3@eventflow.com",
    name: "Third Demo Attendee",
    role: UserRole.ATTENDEE,
    passwordHash,
  });
  ...
  const eventDefinitions = [
    {
      ...
      description: "Technology, AI, software engineering and digital entrepreneurship summit.",
    ...
    },
    {
      ...
      description: "Hands-on developer meetup covering modern web development, APIs and cloud deployment.",
    ...
    },
    {
      ...
      description: "Startup, funding, product and sustainable business growth forum.",
    ...
    {
      title: "Draft Event - Organizer Workspace Demo",
      slug: "draft-event-organizer-workspace-demo",
      description: "Draft event used to test organizer event-management workflows.",
      venue: "EventFlow Demo Venue",
      city: "Zaria",
      startAt: iso(150, 10),
      endAt: iso(150, 14),
    ...
    {
      title: "Cancelled Community Technology Forum",
      slug: "cancelled-community-technology-forum",
      description: "Cancelled event used to test cancelled-event handling.",
      venue: "Kaduna Community Hall",
      city: "Kaduna",
      startAt: iso(30, 11),
      endAt: iso(30, 15),
  ...
  await createSeedOrder({
    reference: "DEMO-PAID-CHECKED-IN-001",
    userId: attendee.id,
    eventId: demoEvent.event.id,
  ...
  await createSeedOrder({
    reference: "DEMO-PAID-ACTIVE-002",
  ...
  await createSeedOrder({
    reference: "DEMO-PENDING-003",
    userId: attendeeThree.id,
    eventId: meetupEvent.event.id,
  ...
  await createSeedOrder({
    reference: "DEMO-REFUNDED-005",
    userId: attendeeTwo.id,
    eventId: startupEvent.event.id,
    ...
    refund: {
      ...
      providerReference: "DEMO-REFUND-PROCESSED-005",
      reason: "Demo processed refund",
    },
  });

  await createSeedOrder({
    reference: "DEMO-REFUND-PENDING-006",
    userId: attendeeThree.id,
    eventId: startupEvent.event.id,
    status: OrderStatus.PAID,
    ticketType: standard,
    ...
    refund: {
      ...
      reason: "Demo pending refund",
    },
  });

  await createSeedOrder({
    reference: "DEMO-REFUND-PROCESSING-007",
    userId: attendee.id,
    eventId: startupEvent.event.id,
    status: OrderStatus.PAID,
    ticketType: standard,
    ...
    refund: {
      ...
      providerReference: "DEMO-REFUND-PROCESSING-007",
      reason: "Demo processing refund",
    },
  });

  await createSeedOrder({
    reference: "DEMO-REFUND-FAILED-008",
    userId: attendeeTwo.id,
    eventId: meetupEvent.event.id,
    status: OrderStatus.PAID,
    ticketType: general,
    ...
    refund: {
      ...
      providerReference: "DEMO-REFUND-FAILED-008",
      reason: "Demo failed refund",
    },
  });
