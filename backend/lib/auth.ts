...
export async function setSession(user: AuthUser) {
  const token = await new SignJWT({ sub: user.id, id: user.id, email: user.email, name: user.name, role: user.role })
    .setProtectedHeader({ alg: JWT_ALGORITHM, typ: "JWT" }).setIssuedAt().setIssuer(JWT_ISSUER).setAudience(JWT_AUDIENCE)
    .setExpirationTime(`${SESSION_MAX_AGE}s`).sign(secret);
...
export async function clearSession() {
  const cookieStore = await cookies();
  cookieStore.set(COOKIE_NAME, "", { ...sessionCookieOptions, maxAge: 0, expires: new Date(0) });
}
