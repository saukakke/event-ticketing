...
type Response = { data: User[]; pagination: { page: number; pageSize: number; total: number; pages: number } };
  ...
  return (
    <main className="container section">
      <div className="dashboard-head">
        <div><div className="eyebrow">Administration</div><h1>Manage users</h1><p className="meta">View platform users and manage their access roles.</p></div>
      </div>
      ...
      <div className="card" style={{ display: "flex", gap: ".75rem", flexWrap: "wrap", marginBottom: "1.5rem" }}>
        ...
        <select className="input" style={{ maxWidth: 220 }} value={role} onChange={(e) => setRole(e.target.value)}>
      ...
      <div className="table-wrap">
        <table>
          <thead><tr><th>User</th><th>Role</th><th>Events</th><th>Orders</th><th>Joined</th><th>Action</th></tr></thead>
          <tbody>
            {data?.data.map((user) => (
              <tr key={user.id}>
                ...
                <td>
                  <select className="input" value={user.role} disabled={saving === user.id} onChange={(e) => void changeRole(user, e.target.value as User["role"])} aria-label={`Change role for ${user.name}`}>
                    <option value="ATTENDEE">Attendee</option><option value="ORGANIZER">Organizer</option><option value="ADMIN">Admin</option>
                  </select>
                </td>
              </tr>
            ))}
      ...
      {data && <p className="meta" style={{ marginTop: "1rem" }}>Showing {data.data.length} of {data.pagination.total} users.</p>}
    </main>
  );
}
